...
<div id="video-control">
	<div class="row pm-video-control">
		<div class="col-xs-5 col-sm-6 col-md-6">
			<button class="btn btn-video {if $bin_rating_vote_value == 1}active{/if}" id="bin-rating-like" type="button" rel="tooltip" data-title="{$video_data.up_vote_count_formatted} {$lang._likes}"><i class="mico mico-thumb_up"></i> <span class="hidden-xs">{$lang._like}</span></button>
			<button class="btn btn-video {if $bin_rating_vote_value == 0 && $bin_rating_vote_value !== false}active{/if}" id="bin-rating-dislike" type="button" rel="tooltip" data-title="{$video_data.down_vote_count_formatted} {$lang._dislikes}"><i class="mico mico-thumb_down"></i></button>
			...
			<div class="row pm-user-header">
				...
				<div class="col-xs-2 col-sm-3 col-md-3">
					...
					<div class="progress" title="{$video_data.up_vote_count_formatted} {$lang._likes}, {$video_data.down_vote_count_formatted} {$lang._dislikes}">
					  <div class="progress-bar progress-bar-success" id="rating-bar-up-pct" style="width: {$video_data.up_pct}%;"></div>
					  <div class="progress-bar progress-bar-danger" id="rating-bar-down-pct" style="width: {$video_data.down_pct}%;"></div>
					</div>
				</div>
			</div><!--/.pm-user-header-->
			...
			<div class="{if ! $playlist && $video_data.video_player_autoplay_next_support}pm-related-with-autoplay{/if} {if $smarty.cookies.pm_autoplay_next == 'on'}with-highlight{else}without-highlight{/if}" id="pm-related">
				...
				<li>
					<div class="pm-video-thumb">
						...
						<div class="pm-video-labels hidden-xs">
							{if $related_video_data.featured}<span class="label label-featured">{$lang._feat}</span>{/if}
						</div>
						<img src="{$smarty.const._URL}/templates/{$template_dir}/img/echo-lzld.png" alt="{$related_video_data.attr_alt}" data-echo="{$related_video_data.thumb_img_url}" class="img-responsive">
						</a>
					</div>
				...
				<li>
					<div class="pm-video-thumb">
						...
						<div class="pm-video-labels hidden-xs">
							{if $popular_video_data.featured}<span class="label label-featured">{$lang._feat}</span>{/if}
						</div>
