	...
	{if $total_video_ads > 0}
		...
		function init() {
			if (document.getElementById) {
				Player = document.getElementById("{/literal}{$video_ad_hash}{literal}-playerholder");
			}
		}
		...
		function onClipDone(clip) {
			if (clip.name == "Advertisment") {
				Player.DoStop();
				swfobject.removeSWF("{/literal}{$video_ad_hash}{literal}-playerholder");
		...
		swfobject.embedSWF("{/literal}{$smarty.const._URL2}{literal}/players/flowplayer2/flowplayer.swf", "{/literal}{$video_ad_hash}{literal}-playerholder", "{/literal}{$smarty.const._PLAYER_W}{literal}", "{/literal}{$smarty.const._PLAYER_H}{literal}", "9.0.115", false, flashvars, params, attributes);
		</script>
		{/literal}
	{else}
				...
				var flashvars = {
						{/literal}
							...
							{elseif $video_data.source_id == 1}
								...
								{/literal}
							{elseif $video_data.source_id == 3}
								{if $video_data.fetch == 0}
								{literal}
								file: '{/literal}{$video_data.direct}{literal}',
								{/literal}
							{else}
								{literal}
								sources: {/literal}{$video_data.direct}{literal},
								{/literal}
							{/if}
							...
							primary: "flash",
							skin:"http://hipmat.com/players/jwplayer6/skins/tred.xml",			
							width: "100%",
							{/literal}{if $playlist}{literal}
							height: "401",
							autostart: true, 
