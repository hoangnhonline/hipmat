...
//					}
					...
					*/
					...
					if (count($items) > 0)
					{
						foreach ($items as $k => $item)
						{
							...
							$item['direct'] = str_replace('youtu.be/', 'youtube.com/watch?v=', $item['direct']);
							$item['direct'] = trim($item['direct']);
	...
	case 'cron':
		...
		{
			if ( ! csrfguard_check_referer('_admin_cron_jobs_form'))
			{
				$ajax_msg = 'Invalid token or session expired. Please refresh this page and try again.';
				exit(json_encode(array(	'success' => false, 
										'alert_type' => 'error',
			...
			$nonce = csrfguard_raw('_admin_cron_jobs_form');
		}
		...
		switch ($action)
		{
			...
			case 'start-job':
				...
				switch ($job['type'])
				{
					...
					case 'vscheck':
						...
						$job['data']['sql_start'] = 0;
						$job['data']['time_started'] = 0;
						$job['data']['videos_processed'] = 0;
						$job['last_exec_time'] = 0;
			...
			case 'add-job':
				...
				$ajax_msg = 'New automated job created successfully. <br />You can now find it in the "Automated Jobs" menu.';
				exit(json_encode(array(	'success' => true, 
										'alert_type' => 'success',
										'msg' => $ajax_msg,
										'html' => pm_alert_success($ajax_msg),
